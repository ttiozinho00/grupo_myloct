...
async function registerUser(userData) {
    let { name, phoneNumber,email, gender, password, repeatPassword } = userData;
    let errors = [];
    ...
    if (/^\([1-9]{2}\)(?:[2-8]|9[1-9])[0-9]{3}[0-9]{4}$/.test(phoneNumber) == false) errors.push('Phone number should be a valid BG number; ' );
...
async function loginUser({ email, password }) {
    ...
    let token = jwt.sign({ _id: user._id, email: user.email, createdSells: user.createdSells.length, avatar: user.avatar }, SECRET);
    return token;
}
